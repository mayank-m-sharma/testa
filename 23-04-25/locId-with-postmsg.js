...
function createChildWindow() {
  childWindow = window.open('', '_blank', `
    ...
    toolbar=no
  `);
  // childWindow.name = locationId; -- This didn't work for me
  ...
  setTimeout(() => {
    if (childWindow) {
      childWindow.postMessage({ type: 'LOCATION_ID', payload: locationId }, '*');
    }
  }, 500);
}
...
function loadReactApp() {
  if (!childWindow) return;
  const scriptUrl = 'https://cdn.jsdelivr.net/gh/mayank-m-sharma/testa@refs/heads/main/23-04-25/fetch-locid-postmsg-rb-01.js';
