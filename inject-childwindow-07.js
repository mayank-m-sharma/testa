...
function createFloatingCallButton({
    ...
    if (injectIntoFlexContainer) {
        // Create a promise that resolves when both elements are found
        ...
        // Set up the observer for any future changes
        const handleDOMChanges = () => {
            waitForElements().then(({ parent, oldChild }) => {
                // Insert our container before the old child
                parent.insertBefore(container, oldChild);
                // Remove the old child
                oldChild.style.visibility = 'none';
            });
        };
        ...
        const observer = new MutationObserver(() => {
            const existingButton = document.contains(container);
            if (!existingButton) {
                handleDOMChanges();
            }
        });
