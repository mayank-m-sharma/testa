...
function createChildWindow() {
  ...
  // Floating launcher button (if not already added)
  if (!document.getElementById('floating-launcher')) {
    const launcher = document.createElement('div');
    launcher.id = 'floating-launcher';
    launcher.style.position = 'fixed';
    launcher.style.top = '50px';
    launcher.style.left = (window.innerWidth - 360) + 'px';
    launcher.style.width = '40px';
    launcher.style.height = '40px';
    launcher.style.borderRadius = '20px';
    ...
    launcher.title = 'Open call window';
    launcher.innerHTML = '📞';
...
function monitorUrlChanges() {
    ...
    setInterval(() => {
        ...
        const contactDetailMatch = currentUrl.match(/\/v2\/location\/([a-zA-Z0-9]+)\/contacts\/detail\/([a-zA-Z0-9]+)/);
...
function initChildWindow() {
    if (isAllowedUrl(window.location.href)) {
        ...
        const contactDetailMatch = window.location.href.match(/\/v2\/location\/([a-zA-Z0-9]+)\/contacts\/detail\/([a-zA-Z0-9]+)/);
        const isContactDetailPage = !!contactDetailMatch;
        const contactId = contactDetailMatch ? contactDetailMatch[2] : null;
