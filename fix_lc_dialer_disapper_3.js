...
// Helper function to show oldChild
function showOldChild() {
  if (oldChildElement) {
    oldChildElement.style.visibility = "visible";
  }
}

// Helper function to hide oldChild
function hideOldChild() {
  if (oldChildElement) {
    oldChildElement.style.visibility = "hidden";
  }
}
  ...
  if (injectIntoFlexContainer) {
    ...
    const handleDOMChanges = () => {
      ...
      waitForElements().then(({ parent, oldChild }) => {
        ...
        hideOldChild(); // Hide oldChild when showing our button
      });
    };
