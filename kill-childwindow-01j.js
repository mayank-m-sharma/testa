...
const allowedPatterns = [
  /\/v2\/location\/([a-zA-Z0-9]+)\/dashboard/,
...
function loadReactApp() {
  if (!childWindow) return;
  const scriptUrl = 'https://ghlsdk.textgrid.com:613/js/react-app-build-prod.js';
...
function monitorUrlChanges() {
  ...
  setInterval(() => {
    ...
    const isStillInSameLocation = currentUrl.includes(`/v2/location/${currentLocationId}/`);
    if (currentUrl !== lastUrl) {
      lastUrl = currentUrl;

      if (!isAllowedUrl(currentUrl)) {
        if (buttonInstance) buttonInstance.remove();
